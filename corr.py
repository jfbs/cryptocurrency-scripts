...
# src = http://www.quantatrisk.com/2017/03/31/cryptocurrency-portfolio-correlation-pca-python/
...
dfP = df[(df.index >= "2017-08-21") & (df.index <= "2017-09-22")]
...
# mycmap.set_under('w')
plt.imshow(co, cmap="RdYlGn", interpolation="nearest") # RdGy
cb = plt.colorbar()
...
plt.imshow(v, cmap="bwr", interpolation="nearest") # bwr
cb = plt.colorbar()
plt.yticks(np.arange(len(tickers)), tickers)
